				...
				<div class="each-tintuc">
					<div class="row">
						<div class="col-md-2 col-sm-3">
							<a href="{{route('user.tintuc.detail',$item->slug)}}"><img src="{{$item->image_path}}" class="img-responsive img-tintuc" alt="{{$item->image_name}}" title="{{$item->image_name}}"></a>
						</div>
						<div class="col-md-10 col-sm-9">
							<h3 class="title-tintuc">{{$item->name}}</h3>
							<p>{{Str::words($item->mota,30)}}</p>
							<p class="readmore"><a href="{{route('user.tintuc.detail',$item->slug)}}">Read more...</a></p>
						</div>
					</div>
				</div>	<!-- end each-tintuc-->
				@endforeach
