...
class Eloquent extends AbstractEloquent implements RepoInterface{
	...
	public function sanpham_danhmuc($slug){
		return $this->sanpham->where('danhmuc_id',$this->danhmuc->where('slug',$slug)->first()->id)->where('status',1)->paginate(12);
	}
	...
	public function tintuc_relate($number,$slug){
		...
		return $this->news->where('status',1)->take($number)->offset($current->id)->get();
	}
